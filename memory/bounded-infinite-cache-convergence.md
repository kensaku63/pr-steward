...
- Rebuilding loaded depth is expensive, so realtime hints for the same scope must be coalesced. Allow one refresh chain in flight, fold overlapping hints into a dirty bit, and run at most one follow-up against the latest snapshot. A generation counter that only discards stale results does not stop the old HTTP and database work.
